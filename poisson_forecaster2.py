...
class PoissonProcessDecomposer2:
    """
    ...
    def detect_holidays(self, threshold: float = 0.5,
        ...
        MODIFICATION 2: Spikes logic based on MAX is disabled to treat single events as noise.
        """
        ...
        # Empty maps / default 1.0
        raw_map = {m: 1.0 for m in range(1, 13)}
        shrunk_clamped = {m: 1.0 for m in range(1, 13)}

        # Store spike parameters (neutralized)
        self.spike_params = {
            "raw_map": raw_map,
            "month_map": shrunk_clamped,
            "threshold": 999.0,  # effectively unreachable
            "scale": 0.0
        }
    ...
    def validate_fit(self) -> Dict[str, float]:
        ...
        # Console output
        print("\n" + "=" * 60)
        print("FIT VALIDATION METRICS (Class 2)")
        print("=" * 60)
...
class KolokwiumForecaster2:
    """
    High-level forecaster built on PoissonProcessDecomposer2.
    ...
    def __init__(self, data_path: str):
        ...
        self.decomposer: Optional[PoissonProcessDecomposer2] = None
    ...
    def decompose(self, trend_method: str = "linear",
        ...
        model = PoissonProcessDecomposer2(df2, date_col="date", value_col="calls")
    ...
    def estimate_monthly_seasonality_on(self, model: PoissonProcessDecomposer2) -> Dict[int, float]:
    ...
    def evaluate_seasonality_methods(self, holdout_days: int = 30) -> Dict[str, Any]:
        ...
        m = PoissonProcessDecomposer2(df_train, date_col="date", value_col="calls")
    ...
    def calibrate_intensity(self, model: PoissonProcessDecomposer2,
    ...
    def forecast_from_decomposer(self, periods: int = 365,
        ...
        # Monthly spike adjustment (Now neutralized to 1.0 by default in Class 2)
        ...
        # Optional multiplicative lognormal noise with HETEROSCEDASTICITY
        if add_noise:
            base_cv = getattr(model, "residual_cv", 0.1)
            used_cv_scalar = max(0.0, min(1.0, base_cv * float(noise_scale)))

            if used_cv_scalar > 1e-4:
                # Increase variance specifically for holidays
                # We assume data is not homoscedastic.
                # Create a vector of CVs, not just a scalar.
                cv_vector = np.full(periods, used_cv_scalar)

                # Identify holidays (where effect is applied, usually < 1.0)
                # If holiday_effect_future is significantly different from 1.0, treat as holiday
                is_holiday_future = (np.abs(holiday_effect_future - 1.0) > 1e-3)

                # Increase CV for detected holidays (e.g. 3x variability)
                holiday_noise_multiplier = 3.0
                cv_vector[is_holiday_future] *= holiday_noise_multiplier

                # Calculate sigma per day
                var_mult_vector = cv_vector ** 2
                sigma_vector = np.sqrt(np.log(1 + var_mult_vector))
                mu_vector = -0.5 * sigma_vector * sigma_vector

                # Generate noise using vector mu and sigma
                noise = rng.lognormal(mean=mu_vector, sigma=sigma_vector, size=periods)
                lam_noisy = lam * noise
...
if __name__ == "__main__":
    ...
    # Initialize forecaster (Class 2)
    f = KolokwiumForecaster2(DATA_PATH)
